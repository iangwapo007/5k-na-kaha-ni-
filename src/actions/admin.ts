...
export async function saveMemorandumLetter(
  departmentId: string,
  ...
  eSignatures: string[],
  formData: FormData
): Promise<ActionResponseType & { memorandumId?: string; letterId?: string }> {
  await connectDB();
  try {
    ...
    if (!!session?.user) {
      ...
      const signatureApprovals = eSignatures.map((signatureId) => ({
      ...
      if (doctype === DocumentType.Memo) {
        ...
        const deptName = dept?.name
          ...
          .map((v: string) =>
            v.length === 1 || /[A-Z]/.test(v)
              ? v?.toUpperCase()
              : v[0]?.toUpperCase()
          )
          .join("");
        const series = `${deptName}${doctype?.[0]?.toUpperCase()}${doctype
          ?.substring(1)
          ?.toLowerCase()}${(count + 1).toString().padStart(3, "0")}_Series`;
        ...
        if (!!memo?._id) {
          if (!!rejectedId) {
            try {
              const deleted = await Memo.deleteOne(
                { _id: rejectedId },
                { runValidators: true }
              ).exec();
          ...
          await Promise.all(
            signatureApprovals.map(async (sa) => {
              try {
                const eSig = await ESignature.findById(sa.signature_id).exec();
                const userSig = await User.findById(
                  eSig.adminId.toHexString()
                ).exec();
...
export async function saveMemorandumLetterToIndividual(
  individualId: string,
  doctype: DocumentType,
  cc: string[],
  formData: FormData
    ...
    if (!!session?.user) {
      ...
      if (doctype === DocumentType.Memo) {
          ...
          try {
            const href =
              individual.role === role
                ? "/" + role + "/memo/receive?id=" + memo._id
                : "/" + Roles.Faculty + "/memo?id=" + memo._id;
        ...
        try {
          const href =
            individual.role === role
              ? "/" + role + "/letter/receive?id=" + letter._id
              : "/" + Roles.Faculty + "/letter?id=" + letter._id;
          await addNotification(individual._id!.toString(), {
            title: "New Letter Sent to you",
            message:
              letter.title +
              " for " +
              individual.firstName +
...
export async function approveMemorandumLetter(
  doctype: DocumentType,
  memoLetterId: string
  ...
  try {
    const session = await getSession(role);
    if (!!session?.user) {
      const eSignature = await ESignature.findOne({
      ...
      if (!!eSignature?._id) {
        ...
        if (doctype === DocumentType.Memo) {
          ...
          memo.signatureApprovals.find(
            (signatureApproval: any) =>
              signatureApproval.signature_id.toHexString() === sid
          ).approvedDate = new Date();
          const updated = await memo.save({
            new: true,
            upsert: false,
            runValidators: true,
          });
          if (!!updated?._id) {
            if (
              JSON.parse(JSON.stringify(updated)).signatureApprovals.every(
                (signatureApproval: any) => !!signatureApproval.approvedDate
              )
            ) {
              const title = "New Memorandum";
              const message = memo.title;
              const href =
                "/" + Roles.Faculty + "/memo?id=" + memo._id.toHexString();
              try {
                await broadcastNotification({
                  role: Roles.Faculty,
                  departmentId: memo.departmentId as string,
                  title,
                  message,
                  href,
                });
              ...
              const hrefAdmin =
                "/" + role + "/memo/approved?id=" + memo._id.toHexString();
              try {
                await broadcastNotification({
                  role: role,
                  departmentId: memo.departmentId as string,
          ...
          letter.signatureApprovals.find(
            (signatureApproval: any) =>
              signatureApproval.signature_id.toHexString() === sid
          ).approvedDate = new Date();
          ...
          if (!!updated?._id) {
            if (
              ...
              const href =
                "/" + Roles.Faculty + "/memo?id=" + letter._id.toHexString();
              try {
                await broadcastNotification({
                  role: Roles.Faculty,
                  departmentId: letter.departmentId as string,
                  title,
                  message,
                  href,
                });
              ...
              const hrefAdmin =
                "/" + role + "/memo/approved?id=" + letter._id.toHexString();
              try {
                await broadcastNotification({
                  role: role,
                  departmentId: letter.departmentId as string,
...
export async function rejectMemorandumLetter(
  doctype: DocumentType,
  memoLetterId: string,
  ...
  try {
    ...
    if (!!session?.user) {
      ...
      if (!!eSignature?._id) {
        const sid = eSignature._id.toHexString();
        if (doctype === DocumentType.Memo) {
          const memo = await Memo.findById(memoLetterId)
          ...
          if (!!updated?._id) {
            ...
            await Promise.all(
              JSON.parse(JSON.stringify(memo)).signatureApprovals.map(
                async (signatureApproval: SignatureApprovals) => {
                  try {
                    const eSig = await ESignature.findById(
                      signatureApproval.signature_id
                    ).exec();
                    const userSign = await User.findById(
                      eSig.adminId.toHexString()
                    ).exec();
          ...
          if (!!updated?._id) {
            ...
            await Promise.all(
              JSON.parse(JSON.stringify(letter)).signatureApprovals.map(
                async (signatureApproval: SignatureApprovals) => {
                  try {
                    const eSig = await ESignature.findById(
                      signatureApproval.signature_id
...
export async function saveESignature(
  id: string | undefined,
...
export async function updateESignature(
  eSignatureDataURL?: string
): Promise<ActionResponseType> {
  await connectDB();
  try {
...
export async function archiveMemorandumLetter(
  doctype: DocumentType,
  id: string,
  ...
  try {
    ...
    if (!!session?.user) {
      ...
      const memoLetterField = isIndividual
        ...
        ? "archivedMemos"
        : "archivedLetters";
      const memo = await User.updateOne(
        { _id: session.user._id },
        { $push: { [memoLetterField]: id } }
      ).exec();
...
export async function unarchiveMemorandumLetter(
  doctype: DocumentType,
  id: string,
  isIndividual: boolean
): Promise<ActionResponseType> {
...
export async function forwardMemorandumLetter(
  memoLetterId: string,
  doctype: DocumentType,
  forwardTo: string,
  ...
  try {
    ...
    if (!!session?.user) {
      ...
      if (doctype === DocumentType.Memo) {
        ...
        if (memo.acknowledged && memo.modifiedCount > 0) {
          try {
            const user = await User.findById(forwardTo)
              .lean<UserDocument>()
              .exec();
