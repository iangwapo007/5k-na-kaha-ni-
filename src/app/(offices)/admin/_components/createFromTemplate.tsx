"use client";
import {
  saveMemorandumLetter,
  saveMemorandumLetterToIndividual,
} from "@/actions/admin";
...
export default function CreateFromTemplate({
  ...
  departmentId?: string;
  individual?: UserDocument;
  template?: TemplateDocument;
  doctype: DocumentType;
  isHighestPosition?: boolean;
  signatoriesList: ESignatureDocument[];
  onSave: (memoradumId: string) => void;
  onCancel: () => void;
}) {
  ...
  const saveMyTemplateIndiv = useMemo(
    () =>
      saveMemorandumLetterToIndividual.bind(
        null,
        individual?._id || "",
        doctype
      ),
  ...
  const saveMyTemplateDept = useMemo(
    () =>
      saveMemorandumLetter.bind(null, departmentId || "", doctype, rejectedId),
    [doctype, departmentId, rejectedId]
  );
  ...
  const getFullName = useCallback((user?: UserDocument): string => {
    const fn =
      ...
      user?.firstName +
      " " +
      (!!user?.middleName ? user?.middleName[0].toUpperCase() + ". " : "") +
      user?.lastName +
  ...
  const fetchAllUsers = useCallback(() => {
    ...
    fetch(url)
      ...
      .then(({ result }) =>
        setOptions(
          result.map((user: UserDocument) => ({
            label: user.employeeId + " - " + getFullName(user),
            value: user._id,
          }))
        )
      )
  ...
  const onSaveAsTemplate = useCallback(() => {
    if (!!individual) {
      Swal.fire({
        ...
        title:
          "Send " +
          (doctype === DocumentType.Memo ? "Memorandum" : "Letter") +
          " to " +
        ...
        text:
          "This " +
          (doctype === DocumentType.Memo ? "Memorandum" : "Letter") +
          " will be sent with your approved signature. Are you sure you want to send?",
        ...
        if (isConfirmed) {
          Swal.fire({
            ...
            if (isConfirmed) {
              ...
              const { success, memorandumId, letterId, error } =
                await saveMyTemplateIndiv(cc, formData);
              if (error) {
                ...
                onSave &&
                  ...
                  onSave(memorandumId as string);
                onSave &&
                  doctype === DocumentType.Letter &&
                  onSave(letterId as string);
              }
            }
          });
        }
      ...
      Swal.fire({
        ...
        text:
          "Once saved, this " +
          (doctype === DocumentType.Memo ? "Memorandum" : "Letter") +
          " will be submitted for approval. Are you sure you want to save?",
        showCancelButton: true,
        ...
        if (isConfirmed) {
          Swal.fire({
            ...
            if (isConfirmed) {
              ...
              formData.append("content", content);
              const { success, memorandumId, letterId, error } =
                await saveMyTemplateDept(cc, eSignatures, formData);
              if (error) {
                toaster.danger(error);
              } else if (success) {
                toaster.success(success);
    ...
    <div className="text-center">
      ...
      <div className="flex items-center justify-center gap-x-4">
        ...
        <button
          type="button"
          onClick={() => onCancel()}
          className="px-2 py-1 rounded bg-gray-300 text-black font-normal text-sm"
        >
          <CrossIcon display="inline" /> Cancel
        </button>
      </div>
      <div className="my-3 text-left px-16 w-full py-2">
        ...
        <SelectMenu
          ...
          onDeselect={(item: any) => {
            const selectedItems = cc.filter(
              (sel_item) => sel_item == item.value
            );
            setCC(selectedItems);
          }}
        >
      ...
      <OCSTinyMCE
        editorRef={editorRef}
        departmentId={!individual ? departmentId : undefined}
        fullName={sessionData?.user?.fullName}
        doctype={!individual ? doctype : undefined}
        signatoriesList={signatoriesList}
        initialContentData={template?.content}
