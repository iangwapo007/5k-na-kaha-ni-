            ...
            {status === 'authenticated' && (
              <>
                ...
                <div className="text-xs text-slate-700 capitalize">
                  {session?.user?.role === Roles.Admin && (session?.user?.highestPosition === HighestPosition.President || session?.user?.highestPosition === HighestPosition.VicePresident) ? session?.user?.highestPosition : (session?.user?.role === Roles.SuperAdmin ? "Admin" : session?.user?.role)}
                </div>
              </div>
